...
def pick_chart(df: pd.DataFrame) -> dict[str, Any] | None:
    ...
    if len(numeric_cols) >= 2 and len(df) > 1:
        ...
        traces = [{"type": "scatter", "mode": "markers", "x": df[x].tolist(), "y": df[y].tolist(), "name": f"{y} vs {x}"}]
...
def _best_time_axis(date_cols: list[str]) -> str:
    # Prefer conventional date/time labels when multiple candidates exist.
    ranked = sorted(
        date_cols,
